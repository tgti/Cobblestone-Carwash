...
*/
@Component({
  selector: 'page-car-wash-services',
  templateUrl: 'car-wash-services.html'
})
export class CarWashServices {
    ...
    constructor(public navCtrl: NavController, public navParams: NavParams, public db: Database, public http: Http, public globalVars: GlobalVars) {
        ...
        this.fav_params = {categories: ["full-service-carwash", "express-exterior"]}; //TODO figure out way to not hard-code this
        this.http.get(`${this.globalVars.api_url}settings`, this.globalVars.api_key_options).map(res => res.json()).subscribe((settings) => {
            settings = settings.settings;
            let setting = settings.carwash_services;
            this.fav_tab = FavoritesTab;
    ...
    ionViewDidLoad() {
        console.log('ionViewDidLoad CarWashServicesPage');
    }
