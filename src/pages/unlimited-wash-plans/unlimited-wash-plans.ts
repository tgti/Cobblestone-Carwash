...
/*
  Generated class for the UnlimitedWashPlans page.
  See http://ionicframework.com/docs/v2/components/#navigation for more info on
  Ionic pages and navigation.
*/
@Component({
  selector: 'page-unlimited-wash-plans',
  templateUrl: 'unlimited-wash-plans.html'
})
export class UnlimitedWashPlans {
    ...
    constructor(public navCtrl: NavController, public navParams: NavParams, public http: Http, public globalVars: GlobalVars) {
        ...
        this.fav_params = {categories: ["ex-unlimited", "fs-unlimited"]}; //TODO figure out way to not hard-code this
        this.http.get(`${this.globalVars.api_url}settings`, this.globalVars.api_key_options).map(res => res.json()).subscribe((settings) => {
            settings = settings.settings;
            let setting = settings.unlimited_wash_plans;
            this.fav_tab = FavoritesTab;
